...
module.exports = {
    async getTechniqueUsersByRoleAndCompany (ctx) {
        try {
            ...
            tasks.forEach(task => {
                console.log("Task assignment date:", dayjs(task.assignment_date).tz('America/Tijuana').subtract(8, 'hour').format('YYYY-MM-DD'));
            });

            const todayTasks = tasks.filter(task => {
                const taskDate = dayjs(task.assignment_date).tz('America/Tijuana').subtract(8, 'hour').format('YYYY-MM-DD');
